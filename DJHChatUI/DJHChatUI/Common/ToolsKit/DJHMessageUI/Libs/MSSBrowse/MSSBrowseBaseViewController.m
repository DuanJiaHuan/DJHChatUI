...
- (void)createBrowseView
{
    ...
    _operationBtn = [[UIButton alloc] initWithFrame:CGRectMake(_screenWidth - 50, _screenHeight - 50, 50, 50)];
    [_operationBtn setImage:[UIImage imageNamed:@"icon_more"] forState:(UIControlStateNormal)];
    [_operationBtn setImageEdgeInsets:UIEdgeInsetsMake(12.5, 12.5, 12.5, 12.5)];
    [_operationBtn addTarget:self action:@selector(operationBtnClick) forControlEvents:(UIControlEventTouchUpInside)];
    [_bgView addSubview:_operationBtn];
...
{
    MSSBrowseCollectionViewCell *cell = (MSSBrowseCollectionViewCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForRow:_currentIndex inSection:0]];
    [self longPress:cell];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {        
        ...
        [cell longPress:^(MSSBrowseCollectionViewCell *browseCell) {
            __strong __typeof(weakSelf)strongSelf = weakSelf;
            if([[SDImageCache sharedImageCache]diskImageExistsWithKey:browseItem.bigImageUrl] || browseItem.bigImageLocalPath.length > 0)
            {
